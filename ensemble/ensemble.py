...
#実行： python /home/kondou/compe2/ensemble/ensemble.py
...
def main():
    ...
    # 出力ファイル名
    OUT_FILE = "ensemble_17,26,23,24.txt"
    
    ens = []    # アンサンブル結果を格納
    for i in range(len(res8)):
        # label = float(res25[i]+res26[i])/2  # luke-large-lite(seed違い) 0.673
        # label = float(res25[i]+res20[i]+res21[i]+res24[i]+res27[i])/5  #largeモデル
        # label = float(res26[i]+res17[i])/2 #0.648と0.648
        # label = float(res17[i]+res22[i]+res13[i]+res26[i]+res23[i]+res24[i]+res36[i]+res35[i]+res40[i]+res21[i]+res30[i]+res31[i]+res37[i]+res39[i])/14 # 全モデル(種類別)
        # label = float(res17[i]+res22[i]+res13[i]+res26[i]+res23[i]+res24[i]+res36[i]+res40[i]+res39[i])/9 #0.600越え
        
        # 順位: 17, 26, 20-25, 13
        # label = float(res17[i]+res26[i]+res20[i])/3 # 1, 1, 3
        # label = float(res17[i]+res26[i]+res25[i])/3 # 1, 1, 3
        # label = float(res17[i]+res26[i]+res13[i])/3 # 1, 1, 4
        # label = float(res17[i]+res26[i]+res25[i]+res13[i])/4 # 1, 1, 3, 4

        # luke-large-lite, luke-large, deberta-v2-base, deberta-v2-large
        # label = float(res25[i]+res26[i]+res23[i]+res24[i])/4
        label = float(res17[i]+res26[i]+res23[i]+res24[i])/4

        label = int(Decimal(str(label)).quantize(Decimal('0'), rounding=ROUND_HALF_UP)) # 四捨五入
        ens.append(label)
