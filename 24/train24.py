...
from transformers import (
    ...
    LukeTokenizer, LukeForSequenceClassification, AutoModelForSequenceClassification,
    pipeline, EarlyStoppingCallback,
    AutoModelForMaskedLM
)

# 実行は：python /home/kondou/compe2/24/train24.py > /home/kondou/compe2/24/1_DeBERTa_train.log

DATA_PATH = "/home/kondou/compe2/data/jumanpp_self_data2/" # dataディレクトリまでのpath (jumanppの都合上顔文字除去済み)
OUT_FNAME = "train24_deberta-v2-large-japanese.txt" # 出力先ディレクトリ
...
def main():
    ...
    train_args = TrainingArguments(
        ...
        learning_rate               = 1e-6, #学習率(デフォルトは5e-5)
    ...
    # モデルの保存
    trainer.save_model("/home/kondou/compe2/24/model") # os.path.join(args.save_model_dir, f"fold{fold_id}"))

    # testに対する推論結果を書き込み
    with open("/home/kondou/compe2/24/"+OUT_FNAME, "w") as f:
        # print(preds)
        for pred in preds:
            # print(pred)
