...
# 実行は：python /home/kondou/compe2/26/train26.py > /home/kondou/compe2/26/1_luke_train.log

DATA_PATH = "/home/kondou/compe2/data/self_data2/" # dataディレクトリまでのpath
OUT_FNAME = "train26_luke-japanese-large_seed222.txt" # 出力先ディレクトリ
...
def main():
    ...
    MyCallback = EarlyStoppingCallback(early_stopping_patience=5)
    ...
    # モデルの保存
    trainer.save_model("/home/kondou/compe2/26/model") # os.path.join(args.save_model_dir, f"fold{fold_id}"))

    # testに対する推論結果を書き込み
    with open("/home/kondou/compe2/26/"+OUT_FNAME, "w") as f:
        # print(preds)
        for pred in preds:
            # print(pred)
